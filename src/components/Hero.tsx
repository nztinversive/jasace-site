...
function HeroSection({ data }: { data: HeroContent }) {
  ...
  useEffect(() => {
    ...
    if (els) {
      setTimeout(() => els.forEach((element) => element.classList.add("visible")), 200);
    }
  }, []);
        ...
        <div className="max-w-2xl space-y-6">
          <div className="reveal">
            <span className="inline-flex items-center gap-3 text-[11px] font-semibold tracking-[0.2em] uppercase text-terra">
              ...
              {data.eyebrow ?? fallbackHeroContent.eyebrow}
            </span>
          </div>
          ...
          <div className="reveal reveal-delay-3 flex flex-wrap items-center gap-3 pt-1">
            <a href={data.ctaLink} className="group px-6 py-3 bg-terra text-stone-50 text-sm font-semibold tracking-wide hover:bg-terra-light transition-colors duration-300 relative overflow-hidden">
            ...
            {data.secondaryCtaText && data.secondaryCtaLink ? (
              <a href={data.secondaryCtaLink} className="px-6 py-3 border border-white/20 text-white/60 text-sm font-semibold tracking-wide hover:bg-white/5 hover:border-white/30 hover:text-white transition-all duration-300">
...
function ConvexHero() {
  const cmsData = useQuery(api.hero.get);
  const data = cmsData ?? fallbackHeroContent;
...
export default function Hero() {
  if (!convexEnabled) {
    return <HeroSection data={fallbackHeroContent} />;
  }
