    ...
    <footer className="bg-stone-950 text-stone-400 pt-20 pb-8">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            {Object.entries(footerLinks).map(([section, links]) => (
              <div key={section}>
                ...
                <ul className="space-y-3">
                  {links.map((link) => (
                    <li key={link}>
                      <a href="#" className="text-sm text-stone-500 hover:text-stone-300 transition-colors">{link}</a>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-stone-600">&copy; {new Date().getFullYear()} Jasace AEC. All rights reserved.</p>
          <div className="flex items-center gap-6">
            ...
            <a href="#" className="text-stone-600 hover:text-stone-400 transition-colors" aria-label="Instagram">
